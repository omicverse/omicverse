...
class TripletMarginMiner(BaseTupleMiner):
    ...
    def __init__(self, margin=0.2, type_of_triplets="all", miner_type='cross_species', **kwargs):
    ...
    def mine(self, embeddings, labels, ref_emb, ref_labels, species, mnn):
        if self.miner_type == 'random':
             anchor_idx, positive_idx, negative_idx = lmu.get_random_triplet_indices(
             labels, ref_labels)
        elif self.miner_type == 'cross_species':
