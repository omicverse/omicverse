...
from ..losses.mixins import EmbeddingRegularizerMixin


class BaseMetricLossFunction(
    EmbeddingRegularizerMixin, ModuleWithRecordsReducerAndDistance
):
    def compute_loss(self, embeddings, labels, indices_tuple=None, embs_list=None):
        """
        This has to be implemented and is what actually computes the loss.
        """
        raise NotImplementedError

    def forward(self, embeddings, labels, indices_tuple=None, embs_list=None):
        """
        Args:
        ...
        loss_dict = self.compute_loss(embeddings, labels, indices_tuple, embs_list)
